...
class TableReservationSystem:
    ...
    def get_available_tables(self, guests_num) -> list[Table]:
        ...
        #sort all tables by seats number
        # s_list = sorted(self.tables, key = lambda x : 'seat')
        # print(s_list, 'jjjj')

        #get only availble table that suit number of ppl:
        filter_table = list(filter(lambda x: x.is_availble and x.seats >= guests_num ,s_list))

        sort_availble = sorted(filter_table, key = lambda x : 'seat')
